...
/* ========================================================
   TOP-K OPTION EXTENSION  (appended to original parse.c)
   ========================================================
   Parses two new command-line options:

     --topk   N     enable sparse top-k pheromone update; use N best ants
                    N=0 disables (reverts to original behaviour)
                    short alias: -T N

   Example usage:
     ./acotsp --mmas -i berlin52.tsp -r 1 -s 10000 -t 30 --topk 5
     ./acotsp --mmas -i berlin52.tsp -r 1 -s 10000 -t 30          # original

   This extension piggybacks on the existing parse_commandline() by
   pre-scanning argv for --topk / -T before the original parser runs.
   This avoids patching the large generated parse_options() function.
   ======================================================== */

#include "topk.h"

/* Called from init_program() AFTER parse_commandline() returns.
   We re-scan argv here so we do not need to modify the opag-generated code. */
    ...
    for (i = 1; i < argc; i++) {
        if ((strcmp(argv[i], "--topk") == 0 || strcmp(argv[i], "-T") == 0)
            ...
            if (val > 0) {
                top_k      = val;
                top_k_flag = 1;
                printf("  --topk %ld: sparse top-k pheromone update ENABLED\n", top_k);
            } else {
                top_k_flag = 0;
                printf("  --topk 0: using original blocking update (default)\n");
            }
            i++; /* skip argument */
        }
    }
    if (!top_k_flag) {
        printf("  Note: --topk not specified; using original blocking update\n");
    }
}
